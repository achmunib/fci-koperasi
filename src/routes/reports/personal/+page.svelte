<script lang="ts">
	...
	import type { PersonalReport as PersonalReportType, DateRange } from '$lib/types';
	import { reportService } from '$lib/services';
	import { authStore } from '$lib/stores/auth';
	import { PersonalReport } from '$lib/components/report';
	...
	let report = $state<PersonalReportType | null>(null);
	...
	async function loadReport() {
		if (!currentUser?.memberId) {
			error = 'Anda harus login sebagai anggota untuk melihat laporan personal';
			loading = false;
			return;
		}
		...
		try {
			loading = true;
			error = null;
			report = await reportService.getPersonalReport(currentUser.memberId, dateRange);
		} catch (err) {
	...
	async function handleExport(format: 'pdf' | 'csv') {
		...
		try {
			exporting = true;
			const blob = await reportService.exportReport('personal', format, {
			...
			a.href = url;
			a.download = `laporan-personal-${currentUser.memberId}-${Date.now()}.${format}`;
...
<div class="max-w-7xl mx-auto space-y-6">
	...
	<div class="flex items-center justify-between">
		<div class="flex items-center gap-4">
			...
			<div>
				<h1 class="text-3xl font-bold text-gray-900">Laporan Personal</h1>
				<p class="text-gray-600 mt-1">Laporan keuangan pribadi Anda</p>
			</div>
		</div>
